...
void WebServer::handleLog(struct mg_connection* c, struct mg_http_message* hm)
{
    /* GET：返回当前级别；POST：body 为级别名（next 表示循环切到下一级） */
    if (mg_strcmp(hm->method, mg_str("GET")) == 0) {
        char buf[64];
        std::snprintf(buf, sizeof(buf), "{\"ok\":true,\"level\":\"%s\"}",
                      Logger::levelName(Logger::instance().level()));
        replyJson(c, buf);
        return;
    }

    char level[16] = "next";
    if (hm->body.len > 0 && hm->body.len < sizeof(level))
        std::snprintf(level, sizeof(level), "%.*s",
                      static_cast<int>(hm->body.len), hm->body.buf);

    LogLevel cur = Logger::instance().level();
    LogLevel nxt = cur;
    if (std::strcmp(level, "next") == 0) {
        nxt = static_cast<LogLevel>((static_cast<int>(cur) + 1) % 5);
        ...
        nxt = LogLevel::kError;
    } else if (std::strcmp(level, "off") == 0) {
        nxt = LogLevel::kOff;
    } else {
        replyJson(c, "{\"ok\":false,\"error\":\"bad level\"}");
        return;
    }
    Logger::instance().setLevel(nxt);
    char buf[80];
    std::snprintf(buf, sizeof(buf), "{\"ok\":true,\"level\":\"%s\"}",
...
void WebServer::handleHttp(struct mg_connection* c, struct mg_http_message* hm)
{
    if (mg_match(hm->uri, mg_str("/"), nullptr) ||
        ...
        handleDisplay(c, hm);
    } else if (mg_match(hm->uri, mg_str("/api/log"), nullptr)) {
        handleLog(c, hm);
    } else {
